...
class Penjualan_detail extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($cari <> '') {
            $config['base_url'] = base_url() . 'penjualan_detail?cari=' . urlencode($cari);
            $config['first_url'] = base_url() . 'penjualan_detail?cari=' . urlencode($cari);
        } else {
            $config['base_url'] = base_url() . 'penjualan_detail';
            $config['first_url'] = base_url() . 'penjualan_detail';
        }
    ...
    public function delete($barang) 
    {
        ...
        unset($_SESSION[$_SESSION['kode'] . 'detailbarang'][$barang]);
        if($this->uri->segment(4)<>"" and $this->uri->segment(4)==1){
